...
void m1step(int dir) {
  int i;
  //for(i = posx; i <= posx +  noStep; i++){
  if (dir > 0) PORTA = 16 * vstep[(mx1 + 1) % 4];
  else PORTA = 16 * vstep[3 - (mx1 + 1) % 4];
  mx1++;
  delay(DELAYvar);
  //}
  PORTA = 0;
...
void m2step(int dir) {
  ...
  if (dir > 0) PORTA = vstep[(my2 + 1) % 4];
  else PORTA = vstep[3 - (my2 + 1) % 4];
  my2++;
  delay(DELAYvar);
